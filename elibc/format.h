 ...
 *
 * Always use the e-prefixed wrappers provided here:
 *   eprintf(fmt, ...)          — like printf
 *   efprintf(stream, fmt, ...) — like fprintf
 *   esprintf(buf, fmt, ...)    — like sprintf
 *   esnprintf(buf, n, fmt, ...) — like snprintf
 *
 * Example (CORRECT):
 *   int age = 15;
 *   eprintf("Your age is %int\n", age);   // → "Your age is 15"
 *
 * Example (WRONG — do not do this):
 *   printf("Your age is %int\n", age);    // → "Your age is 15nt"
 *
 * For compile-time adjacent-literal usage, use the FMT_* constants
 * with standard printf — they expand to valid standard specifiers:
 *   printf("age=" FMT_INT "\n", age);     // fine, expands to "%d"
 */
